...
ritorna T."
  (cond ((not (gethash (list graph-id vertex-id) *visited*))
         (let ((old-key 
                (heap-current-key (heap-actual-heap graph-id) 
                                  (heap-size graph-id) vertex-id)))
           (cond ((not (null old-key)) 
                 ...
                 ((numberp head)
                  (cond
                   ((= (car (cdr (cdr arc))) head)
                    (mst-update-node graph-id (car (cdr (cdr (cdr arc)))) head 
                                     (car (cdr (cdr (cdr (cdr arc)))))))
                   (t (mst-update-node graph-id (car (cdr (cdr arc))) head
                                       (car (cdr (cdr (cdr (cdr arc)))))))))
                 (t (cond
...
vertice source e ritorna NIL."
  ...
  (setf 
   (gethash (list graph-id source) *vertex-key*) 
   most-positive-double-float)
...
preordine."
  ...
  (mst-preorder-tree graph-id source 
                     (sort 
                      (mst-get-childs graph-id source) 
                      'mst-sort-childs) ()))
