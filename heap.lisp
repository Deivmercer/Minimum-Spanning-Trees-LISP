...
;; TODO: Forse c'è da implementare anche heap-id? (e nel caso heap-size ed 
;;       heap-actual-heap potrebbbero essere sbagliate,
;;       in quanto dovrebbero prendere non l'id ma heap-rep)
;;       https://elearning.unimib.it/mod/forum/discuss.php?d=148950#p252156
;; (defun heap-id (?))
  ...
  (cond
   ...
   (t (let ((current (aref (heap-actual-heap heap-id) index))
        ...
        (cond ((< (car current) (car parent)) 
               ...
               (setf 
                (aref (heap-actual-heap heap-id) (floor (/ index 2))) 
                current)
    ...
    (setf
     ...
     (list 'heap heap-id 
           ...
           (adjust-array 
            ...
            (+ (heap-size heap-id) 1))))))
  (setf (aref (heap-actual-heap heap-id) (heap-size heap-id)) (list K V))
  (setf
   (gethash heap-id *heaps*)
   (list 'heap heap-id (+ (heap-size heap-id) 1) (heap-actual-heap heap-id)))
...
(defun heapify (heap index size)
  ...
  (cond ((/= index -1) 
         (heapify heap 
                  (swap heap index 
                        (smallest heap (- (+ (* (+ index 1) 2) 1) 1)
                                  (smallest heap (- (* (+ index 1) 2) 1) index 
                                            size) 
                                  size))
                  size))))
  ...
  (cond
   ...
   (t (mapheap 
       #'(lambda (heap-entry index) 
           (cond ((= (car heap-entry) old-key)
                  (cond
                   ((numberp V) 
                    (cond ((= (car (cdr heap-entry)) V)
                           (setf 
                   ...
                   (t (cond ((string= (car (cdr heap-entry)) V)
                             (setf 
                              (aref (heap-actual-heap heap-id) index) 
                              (list new-key V))
                             (cond
...
(defun heap-current-key (heap size value &optional (index 0))
  "Ritorna la chiave dell'elemento con valore V all'interno dell'heap"
  (cond ((< index size)
         (cond 
          ((numberp value)
           (cond 
            ((= value (car (cdr (aref heap index)))) (car (aref heap index)))
            (t (heap-current-key heap size value (+ index 1)))))
          (t (cond
              ((string= value (car (cdr (aref heap index)))) 
               (car (aref heap index)))
              (t (heap-current-key heap size value (+ index 1)))))))))
